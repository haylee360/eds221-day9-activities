...
mack_creek_lengths <- mack_creek_vertebrates %>% 
  ...
  summarize(mean_length_cm = mean(length1, na.rm = TRUE),
            sd_length_cm = sd(length1, na.rm = TRUE)) %>% 
  ungroup()
#> Error in mack_creek_vertebrates %>% clean_names() %>% select(year:sampledate) %>% : could not find function "%>%"

# Idk why the reprex is showing a different error than what pops up when I run the chunk?
```

```{r}
#Trying with minimum info
mack_creek_lengths <- mack_creek_vertebrates %>% 
  mutate(across(where(is.character, tolower)))

mack_creek_lengths <- mack_creek_vertebrates %>% 
  mutate(across(where(is.character, tolower)))
#> Error in mack_creek_vertebrates %>% mutate(across(where(is.character, : could not find function "%>%"
...
mack_creek_vertebrates %>% 
  ...
  ggplot(aes(x = length1, y = weight)) +
         ...
         labs(x = "Cutthroat trout length (cm)",
              y = "Weight (g)")
#> Error in mack_creek_vertebrates %>% clean_names() %>% filter(species == : could not find function "%>%"

# Again, same issue as above reprex 
```
