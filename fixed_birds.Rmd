...
```
*Error in here("knb-lter-cap_256_9") : could not find function "here"* --> loaded library here() in the set up chunk
...
head(taxalist_1)
```
*Error in species_id(.) : could not find function "species_id"* --> use "$" instead of %>% to pull a column from a data frame
...
```
*Join columns in `x` must be present in the data. ✖ Problem with `species`.* --> used wrong column name, change to species_id
...
```
*Error in groupby(., park_name) : could not find function "groupby"* -->  fixed typo, group_by()
...
ggplot(data = bird_totals_by_park, aes(x = total_count, y = park_name)) +
  ...
  theme_minimal()
```
*Error in base_size/2 : non-numeric argument to binary operator* --> used %>% instead of + to add the theme
...
```
*Error in `group_by()`: ! Must group by variables found in `.data`. ✖ Column `park_name` is not found.* --> use left_join instead of semi_join
...
```
*`geom_col()` requires the following missing aesthetics: x and y* --> didn't put x and y in aes() in plot setup 
...
```
*Error in f(...) : unused argument (heigth = 6)* --> typo in height
